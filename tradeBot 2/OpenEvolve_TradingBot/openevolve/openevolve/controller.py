...
# Add this line at the top of your local controller.py run() function:
print("🔥 DEBUG: Using LOCAL controller.py..... ")
...
class OpenEvolve:
    ...
    def __init__(
        ...
        if config is not None:
            # Use provided Config object directly ------> Stores my openevolve_config.yaml (added)
            self.config = config
        else:
        ...
        # <-- Uses MY YAML prompt section (added) .... 
    ...
    async def run(
        ...
        for i in range(start_iteration, total_iterations):
            ...
            try:
                ...
                # # Log prompts --> it was there originally 
                # self.database.log_prompt(
                #     template_key=(
                #         "full_rewrite_user" if not self.config.diff_based_evolution else "diff_user"
                #     ),
                ...
                # CRITICAL FIX: Only add successfully evaluated programs to database
                #With the current code, if a strategy generates zero trades, here's exactly what happens: 
                # When OpenEvolve creates an evolved strategy and runs it through freqtrade backtesting, 
                # the strategy produces zero trades (meaning it never finds conditions to buy or sell). 
                # Your trading_strategy_evaluator.py detects this during metrics extraction and 
                # immediately returns {"IsSuccessfulEval": False, "overall_fitness": 0.0, "profit_total_pct": -999.0} -
                # marking it as a complete failure.
                # When this failed result reaches this controller.py, the 
                # if child_metrics.get('IsSuccessfulEval')==1.0000 check evaluates to False, 
                # so the strategy is never added to the database, never considered for "best" 
                # selection, and never used as a parent for future evolution. Instead, it gets 
                # logged as "❌ Rejected failed program" and the iteration continues to the next 
                # attempt. This creates a strict filter ensuring that only strategies that 
                # actively trade can survive the evolutionary process, thus 
                # forcing OpenEvolve to evolve toward active, profitable trading strategies 
                # rather than passive "do nothing" approaches that achieve 0% returns by simply
                # avoiding all trades.
                
                if child_metrics.get('IsSuccessfulEval') > 0.5:
                # hhhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkhkh    
                    print("for this child, IsSuccessfulEval > 0.5 👹👹👹👹👹👹👹👹👹👹👹👹👹👹👹👹👹👹👹")
                    # Create a child program
                    child_program = Program(
                        id=child_id,
                        code=child_code,
                    ...
                    # Add to database (only successful programs)
                    self.database.add(child_program, iteration=i + 1)
                    
                    # Log prompts for successful programs
                    self.database.log_prompt(
                        template_key=(
                            "full_rewrite_user" if not self.config.diff_based_evolution else "diff_user"
                        ),
                    ...
                    all_successful_programs = [p for p in self.database.programs.values() 
                                          if p.metrics.get('IsSuccessfulEval')>0.5]

                    if all_successful_programs:
                        # Find the actual best program by fitness
                        actual_best = max(all_successful_programs, 
                                        key=lambda p: p.metrics.get('overall_fitness', 0))
                        
                        # Update database to track the correct best program
                        if self.database.best_program_id != actual_best.id:
                            old_best_id = self.database.best_program_id
                            self.database.best_program_id = actual_best.id
                            
                            logger.info(f"🔧 CORRECTED: Updated best program from {old_best_id} to {actual_best.id}")
                            logger.info(f"New best fitness: {actual_best.metrics.get('overall_fitness', 0):.4f}")
                        
                        # Only announce "New best" if this child is actually the best AMONG ALL THE PREVIOUSLY SUUCESSFULLY EVALUATED STRATEGIES ...... 
                        if child_program.id == actual_best.id:
                            logger.info(f"🌟🌟🌟🌟🌟 ✅✅✅✅ New best solution found at iteration {i+1}: {child_program.id}")
                            # logger.info(f"Metrics: {format_metrics_safe(child_program.metrics)}")
                    
                    logger.info(f"✅ Added successful program {child_id} with fitness {child_metrics.get('overall_fitness', 0):.4f}")

                else:
                        # Store artifacts for failed programs too (for debugging)
                        if artifacts:
                            self.database.store_artifacts(child_id, artifacts)
                        
                        # Log failed program but don't add to database
                        logger.info(f"❌ Rejected failed program {child_id}: {child_metrics.get('error', 'Failed evaluation')}")
        ...
        if self.database.best_program_id:
            ...
            # CRITICAL: Verify the tracked best is actually successful
            if best_program and best_program.metrics.get('IsSuccessfulEval')>0.5:
                logger.info(f"Using tracked best program: {self.database.best_program_id}")
            else:
                logger.warning(f"Tracked best program {self.database.best_program_id} is invalid or failed")
                best_program = None

        # Fallback to calculating best program if tracked program not found or invalid
        if best_program is None:
            # Only consider successful programs for best selection
            all_programs = [p for p in self.database.programs.values() 
                        if p.metrics.get('IsSuccessfulEval') > 0.5]
            
            if all_programs:
                # Find program with highest overall_fitness among successful programs
                best_program = max(all_programs, 
                                key=lambda p: p.metrics.get('overall_fitness', 0))
                logger.info(f"Using calculated best successful program: {best_program.id}")
                logger.info(f"Selected from {len(all_programs)} successful programs")
            else:
                logger.warning("No successful programs found during evolution")

        # Double-check: if we have a program, make sure it's successful
        if best_program and not (best_program.metrics.get('IsSuccessfulEval')>0.5):
            logger.error(f"Best program {best_program.id} has IsSuccessfulEval=False - rejecting")
            best_program = None

        # Final result
        if best_program:
            fitness = best_program.metrics.get('overall_fitness', 0)
            profit = best_program.metrics.get('profit_total_pct', 0)
            trades = best_program.metrics.get('trades', 0)
            
            logger.info(f"Evolution complete. Best program has:")
            logger.info(f"  - ID: {best_program.id}")
            logger.info(f"  - Fitness: {fitness:.4f}")
            logger.info(f"  - Profit: {profit:.2f}%")
            logger.info(f"  - Trades: {trades}")
            logger.info(f"  - IsSuccessfulEval: {best_program.metrics.get('IsSuccessfulEval')}")
            
            # Save the best program
            self._save_best_program(best_program)
            return best_program
        else:
            logger.error("EVOLUTION FAILED: No successful programs found")
            logger.error("Check your evaluation criteria - they may be too restrictive")
            return None
