...
class RandomStrategy(IStrategy):
    ...
    # Optimized ROI strategy
    minimal_roi = {
        "0": 0.04,     # 4% immediate profit
        "15": 0.03,    # 3% after 15 minutes
        "45": 0.02,    # 2% after 45 minutes
        "90": 0.015,   # 1.5% after 90 minutes
        "180": 0.01    # 1% after 3 hours
    }

    # Enhanced dynamic stoploss
    stoploss = -0.035  # Tighter 3.5% stoploss
    trailing_stop = True
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        price_near_lower = (dataframe['close'] > dataframe['gauss_lower'] * 0.99).astype(int)
        
        # Enhanced stochastic conditions
        stoch_crossover = (
            (dataframe['stoch_k'] <= dataframe['stoch_d']) & 
            (dataframe['stoch_k'].shift(1) > dataframe['stoch_d'].shift(1)) &
            (dataframe['stoch_k'] < 40)  # More conservative oversold level
        ).astype(int)
        
        # Trend confirmation
        upward_pressure = (dataframe['close'] > dataframe['close'].rolling(5).mean()).astype(int)
        
        # Enhanced volume filters
        has_volume = (dataframe['volume'] > 0).astype(int)
        volume_increase = (
            (dataframe['volume'] > dataframe['volume'].rolling(10).mean() * 1.8) &
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        price_weakness = (
            (dataframe['close'] < dataframe['close'].rolling(3).mean()) &
            (dataframe['close'].shift(1) > dataframe['close'].rolling(3).mean().shift(1))
        ).astype(int)
        
        stoch_overbought = (dataframe['stoch_k'] > 75).astype(int)  # More responsive overbought level
        stoch_cross_down = (
            (dataframe['stoch_k'] >= dataframe['stoch_d']) & 
            (dataframe['stoch_k'].shift(1) < dataframe['stoch_d'].shift(1))
        ).astype(int)
        
        volume_decline = (dataframe['volume'] < dataframe['volume'].rolling(5).mean() * 0.7).astype(int)

        dataframe["exit_long"] = (
            price_above_upper | 
            (stoch_overbought & stoch_cross_down) |
            (price_weakness & volume_decline)
        )
        return dataframe
