...
class RandomStrategy(IStrategy):
    ...
    # More aggressive ROI structure for quick profits
    minimal_roi = {
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["enter_long"] = (
            ...
            (dataframe['close'] > dataframe['close'].rolling(window=5).mean()) &  # Short-term uptrend
            (dataframe['volume'] > dataframe['volume'].rolling(window=10).mean() * 1.5)  # Strong volume confirmation
        ).astype(int)
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        trailing_exit = (dataframe['close'] < dataframe['gauss_mid'].rolling(window=3).mean())
        profit_exit = (price_above_upper & stoch_overbought & 
                      (dataframe['volume'] > dataframe['volume'].rolling(window=10).mean()))
        
        dataframe["exit_long"] = (
            profit_exit |  # Normal profit exit
            trailing_exit |  # Trailing exit
            (dataframe['close'] < dataframe['gauss_lower'] * 0.98)  # Emergency stop
        ).astype(int)
        return dataframe
